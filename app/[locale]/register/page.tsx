...
export default function RegisterPage() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 sm:px-6 relative overflow-hidden py-8 sm:py-10">
      {/* Background Decor */}
      ...
      <div className="w-full max-w-2xl relative z-10 animate-in fade-in zoom-in duration-700">
        ...
        <div className="text-center mb-8 sm:mb-10">
          ...
          <p className="mt-2 sm:mt-3 text-sm sm:text-base text-slate-500 font-medium italic">
            {t('register.subtitle')}
          </p>
        </div>

        {/* Card Form */}
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] sm:rounded-[3.5rem] shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 p-4 sm:p-12">
          <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
              <div className="space-y-4 sm:space-y-6">
                 <div className="space-y-3 sm:space-y-4">
                    <Label className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1 flex items-center gap-2">
              ...
              <div className="space-y-4 sm:space-y-6">
                 ...
                 <div className="space-y-3 sm:space-y-4">
                    ...
                    <Input
                      ...
                      disabled={isLoading}
                    />
                    <p className="text-[8px] sm:text-[10px] font-bold text-slate-400 pl-1 uppercase tracking-tighter italic">Min length: 8 chars</p>
                 </div>
              </div>
            </div>

            <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800">
               <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-indigo-600">Region:</span>
            ...
            >
              {isLoading ? (
                ...
                <>
                  {t('register.submit')}
                  <ArrowLeft className="ml-3 h-4 w-4 sm:h-5 sm:w-5" />
                </>
              )}
            </Button>
