...
import { getRecipes, type RecipeDTO, type RecipeLanguage } from '@/lib/api/recipes'
...
function calculateRecommendedPrice(totalCost: number, targetMargin = 0.35): number {
...
export default function CreateDishPage() {
  ...
  useEffect(() => {
    ...
    getRecipes(accessToken, { per_page: 200 })
      .then(res => setRecipes(res.recipes))
      .catch(() => setRecipes([]))
      .finally(() => setRecipesLoading(false))
  }, [user, router, locale, accessToken])

  const handleAddComponent = useCallback(() => {
    setComponents(prev => {
      const newId = (Math.max(...prev.map(c => parseInt(c.id)), 0) + 1).toString()
      return [...prev, { id: newId, recipeId: null, recipeName: '', searchValue: '' }]
    })
  }, [])
  ...
  const handleSearch = useCallback((query: string, index: number) => {
    setComponents(prev =>
      prev.map((comp, i) => i === index ? { ...comp, searchValue: query } : comp)
    )
    setActiveSearchIndex(query.length >= 2 ? index : null)
  }, [])

  const handleSelectRecipe = useCallback((index: number, recipeId: string, recipeName: string) => {
    setComponents(prev =>
      prev.map((comp, i) => i === index ? { ...comp, recipeId, recipeName, searchValue: '' } : comp)
    )
    setActiveSearchIndex(null)
  }, [])
  ...
  const handleClearSelection = useCallback((index: number) => {
    setComponents(prev =>
      prev.map((comp, i) => i === index ? { ...comp, recipeId: null, recipeName: '', searchValue: '' } : comp)
    )
  }, [])
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (!dishName.trim()) {
      setValidationError('Enter dish name')
      return
    }
    const validComponents = components.filter(c => c.recipeId)
    if (validComponents.length === 0) {
      setValidationError('Add at least one recipe')
      return
    }
    const priceVal = parseFloat(salePrice)
    if (!salePrice || priceVal <= 0) {
      setValidationError('Enter sale price')
      return
    }
    ...
    const componentsWithCost = validComponents.map(comp => {
      ...
      const costPerServing = (recipe?.cost ?? 0) / servings
      return {
    ...
    const foodCostPercent = priceVal > 0 ? (totalCost / priceVal) * 100 : 0
    const status = getStatus(marginPercent)
    const aiRecommendedPrice = calculateRecommendedPrice(totalCost)

    const insights: string[] = []
    if (status === 'loss') {
      insights.push(`Warning: Unprofitable dish! Recommended price from ${aiRecommendedPrice.toFixed(2)} PLN`)
    } else if (status === 'warning') {
      insights.push(`Low margin. Optimal price: ${aiRecommendedPrice.toFixed(2)} PLN`)
    } else {
      insights.push(`Good profitability`)
    }
    if (foodCostPercent > 40) {
      insights.push('Food cost above 40% — consider optimization')
    }
    ...
    setPreviewDish({
      ...
      aiRecommendedPrice,
      aiInsights: insights,
      warnings: marginPercent < 20 ? ['Low margin. Consider adjusting the price.'] : []
    })
  ...
  const handleSave = useCallback(async () => {
    if (!previewDish || !accessToken) return
    setSaveError('')
    setMode('saving')

    try {
      const primaryRecipeId = previewDish.components[0]?.recipeId
      if (!primaryRecipeId) throw new Error('No recipe selected')

      await createDish({
        name: previewDish.name,
        recipe_id: primaryRecipeId,
        selling_price_cents: Math.round(previewDish.salePrice * 100),
        image_url: imagePreview ?? null,
      }, accessToken)

      router.push(`/${locale}/dishes`)
    } catch (err) {
      setSaveError(err instanceof Error ? err.message : 'Failed to save dish')
      setMode('preview')
    }
  }, [previewDish, accessToken, imagePreview, router, locale])
          ...
          <div className="space-y-10">
                      ...
                      {previewDish.components.map((comp, i) => (
                        <div key={i} className="flex items-center justify-between p-5 bg-black/40 rounded-2xl border border-white/5">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-white/20">{i + 1}</div>
                    ...
                    <div className="bg-black/40 rounded-[2rem] border border-indigo-500/20 p-8 space-y-6">
                      {previewDish.aiInsights.map((insight, i) => (
                        <div key={i} className="flex items-start gap-4">
                          ...
                          <p className="text-sm font-medium italic text-white/80 leading-relaxed pt-1">{insight}</p>
                        </div>
                      ))}
                        ...
                        <div className="pt-6 border-t border-white/5 space-y-4">
                          {previewDish.warnings.map((warning, i) => (
                            <div key={i} className="flex items-start gap-4 text-rose-400/80">
                              ...
                              <p className="text-xs font-bold uppercase tracking-widest leading-relaxed">{warning}</p>
                            </div>
                          ))}
                        </div>
                      )}
            ...
            {saveError && (
              <div className="bg-rose-500/10 border border-rose-500/20 p-6 rounded-2xl flex items-center gap-4">
                ...
                <p className="text-sm font-black uppercase tracking-widest text-rose-500">{saveError}</p>
              </div>
            )}
        ...
        <div className="space-y-12">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="flex items-center gap-6">
              <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-emerald-500 rounded-[1.5rem] blur opacity-25" />
          ...
          <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-[3rem] p-10 space-y-12">
            ...
            <div>
              ...
              {imagePreview ? (
                <div className="relative w-32 h-32">
                  ...
                  <img src={imagePreview} alt="preview" className="w-32 h-32 rounded-2xl object-cover border border-white/10" />
                  <button
                    onClick={() => setImagePreview(null)}
                    className="absolute -top-2 -right-2 h-6 w-6 bg-black border border-white/10 rounded-full flex items-center justify-center text-white/40 hover:text-rose-500"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ) : (
                <label className="flex items-center gap-4 h-16 px-8 rounded-2xl bg-black/40 border border-white/10 border-dashed cursor-pointer hover:border-indigo-500/50 transition-colors">
                  <Zap className="w-5 h-5 text-white/20" />
                  <span className="text-white/30 font-medium italic text-sm">Upload image...</span>
            ...
            <div className="space-y-6">
              ...
              {recipesLoading && (
                <div className="flex items-center gap-3 px-4 text-white/30">
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span className="text-[11px] font-black uppercase tracking-widest">Loading recipes...</span>
                </div>
              )}
                            ...
                            {searchResults.map((recipe) => (
                              ...
                              >
                                <div className="text-left">
                                  <p className="font-black italic text-white">{recipe.name}</p>
            ...
            <Button
              ...
              disabled={isAnalyzing || recipesLoading}
