...
export function BusinessMetrics({
  ...
  return (
    <div className={cn("space-y-4 sm:space-y-6", className)}>
      <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-3xl sm:rounded-[2.5rem] p-4 sm:p-8">
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-1">
              <div className="p-2 sm:p-2.5 bg-indigo-500/10 rounded-lg sm:rounded-xl border border-indigo-500/20">
                <BarChart3 className="h-4 w-4 sm:h-5 sm:w-5 text-indigo-400" />
              </div>
              <h2 className="text-lg sm:text-xl font-black italic text-white tracking-tight">
                {t('title')}
              </h2>
            </div>
            <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] text-white/40 ml-10 sm:ml-12">
              {t('subtitle')}
            </p>
          </div>
          <Badge variant="outline" className="px-2 sm:px-4 py-1 sm:py-1.5 rounded-full border-white/10 bg-white/5 text-white/60 font-black uppercase tracking-widest text-[7px] sm:text-[10px]">
            Live Analytics
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="bg-black/40 rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 border border-white/5 group hover:border-indigo-500/30 transition-all">
            ...
            <h3 className="text-[9px] sm:text-[11px] font-black uppercase tracking-widest text-indigo-400/80 mb-2">
              {t('cost.title')}
            </h3>
            <div className="flex items-center gap-2 text-[8px] sm:text-[10px] text-white/40 font-medium">
              <Info className="w-2.5 h-2.5 sm:w-3 h-3" />
              {t('cost.efficiency')}
            </div>
          </div>

          {/* Margin Card */}
          <div className="bg-emerald-500/[0.03] rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 border border-emerald-500/10 group hover:border-emerald-500/30 transition-all">
            <div className="flex justify-between items-start mb-3 sm:mb-4">
              <div className="p-2 sm:p-3 bg-emerald-500/10 rounded-xl sm:rounded-2xl">
                <PieChart className="w-4 h-4 sm:w-5 h-5 text-emerald-400" />
              </div>
              <div className="text-right">
                <div className="text-2xl sm:text-3xl font-black text-emerald-400 italic tracking-tighter">
                  {margin}%
                </div>
